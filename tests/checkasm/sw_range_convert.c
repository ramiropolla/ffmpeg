...
#define INPUT_SIZES 6
    ...
    for (int pfi = 0; pfi < FF_ARRAY_ELEMS(pixel_formats); pfi++) {
        ...
        const char *bit_depth_str = bit_depth == 16 ? "16" : "";
    ...
    for (int dstWi = 0; dstWi < INPUT_SIZES; dstWi++) {
        ...
        if (check_func(ctx->lumConvertRange, "%s%s_%d", func_str, bit_depth_str, width)) {
            call_ref(dst0, width);
            call_new(dst1, width);
...
#define INPUT_SIZES 6
    ...
    for (int dstWi = 0; dstWi < INPUT_SIZES; dstWi++) {
        ...
        ff_sws_init_scale(ctx);
        if (check_func(ctx->chrConvertRange, "%s%s_%d", func_str, bit_depth_str, width)) {
