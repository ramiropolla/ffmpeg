...
static void check_convert(const char *name, const SwsUOp *uop)
{
    const SwsPixelType dst = uop->par.convert.to;
...
#define CHECK_MACRO(MACRO, NAME, ...) \
    MACRO(#NAME, &(SwsUOp) { __VA_ARGS__ });

#define CHECK_FOR(UOP, MACRO) \
    SWS_FOR_STRUCT(UOP, U8,  CHECK_MACRO, MACRO) \
    SWS_FOR_STRUCT(UOP, U16, CHECK_MACRO, MACRO) \
    SWS_FOR_STRUCT(UOP, U32, CHECK_MACRO, MACRO) \
    SWS_FOR_STRUCT(UOP, F32, CHECK_MACRO, MACRO) \
    report(#UOP)
...
void checkasm_check_sw_ops(void)
{
    ...
    CHECK_FOR(CONVERT,          check_convert);
