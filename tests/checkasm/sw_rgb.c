...
{
    static const int input_sizes[] = {16, 128, 256, MAX_LINE_SIZE, -MAX_LINE_SIZE};
    ...
    for (int isi = 0; isi < FF_ARRAY_ELEMS(input_sizes); isi++) {
        ...
        if (negstride) {
            ...
            dst_u_0 += (height - 1) * (linesize / 4);
            dst_u_1 += (height - 1) * (linesize / 4);
            dst_v_0 += (height - 1) * (linesize / 4);
            dst_v_1 += (height - 1) * (linesize / 4);
            linesize *= -1;
        }
        ...
        if (check_func(ff_rgb24toyv12, "rgb24toyv12_%d_%d%s", width, height, negstride_str)) {
            ...
            call_new(src1, dst_y_1, dst_u_1, dst_v_1, width, height,
                     linesize, linesize / 2, linesize * 3, ctx->input_rgb2yuv_table);
            if (memcmp(buf_y_0, buf_y_1, BUFSIZE) ||
                memcmp(buf_u_0, buf_u_1, BUFSIZE / 4) ||
                memcmp(buf_v_0, buf_v_1, BUFSIZE / 4))
                fail();
