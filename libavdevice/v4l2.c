...
#endif
    {
        /* CPIA and OV518 are compressed formats and we don't know the
         * exact number of bytes used by a frame, so set it here as the
         * driver announces it. */
        if (ctx->video_codec_id == AV_CODEC_ID_CPIA ||
            ctx->video_codec_id == AV_CODEC_ID_OV518)
