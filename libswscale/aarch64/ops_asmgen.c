                             ...
                             SwsAArch64OpRegs *regs)
{
    ...
    RasmOp *dl = regs->dl;
    RasmOp *dh = regs->dh;
    /* CPS aliases this to tmp0 (freshly loaded from impl->priv every
     * call, in asmgen_setup_dither()); JIT points it at a persistent
     * GPR loaded once before the loop (ops_jit.c). */
    RasmOp src_ptr = s->dither_src_ptr;
