...
{
    RasmContext *r = s->rctx;
    RasmOp nibble_mask = v_8b(s->vk[0]);
    AArch64VecViews vl[1] = { a64op_vec_views(s->vl[0]) };
    AArch64VecViews vtmp  = a64op_vec_views(s->vt[0]);
...
static void asmgen_op_unpack(SwsAArch64Context *s, const SwsAArch64OpImplParams *p)
{
    ...
    LOOP_MASK(p, i) {
        ...
        if (!mask_val[i]) {
            ...
            if (val <= 0xff || val == 0xffff) {
                i_movi(r, vk[cur_vt], IMM(val));
            } else {
                i_mov (r, mask_gpr,   IMM(val));
                i_dup (r, vk[cur_vt], mask_gpr);
            }
            mask_val[i] = val;
            mask_idx[i] = cur_vt++;
        }
    ...
    LOOP_MASK_BWD      (p, i) { i_and(r, vl[i], vl[i], vk[mask_idx[i]]); CMTF("vl[%u] &= 0x%x;", i, mask_val[i]); }
    LOOP_MASK_BWD_VH(s, p, i) { i_and(r, vh[i], vh[i], vk[mask_idx[i]]); CMTF("vh[%u] &= 0x%x;", i, mask_val[i]); }
}
