...
static void asmgen_process(SwsAArch64Context *s, SwsCompMask imask, SwsCompMask omask)
{
    ...
    a64frame_emit(r, &s->frame, prologue, epilogue);
}
                               ...
                               SwsAArch64OpRegs *regs)
{
    ...
    if (p->block_size == 16) {
        ...
        i_and (r, dl[0].b16,   dl[0].b16, bitmask_vec);     CMT("vl[0] &= bitmask_vec;");
    } else {
        i_ldrb(r, wtmp,        a64op_post(s->in[0], 1));    CMT("uint8_t tmp = *in[0]++;");
        i_dup (r, dl[0].b8,    wtmp);                       CMT("vl[0].lo = broadcast(tmp);");
        i_ushl(r, dl[0].b8,    dl[0].b8,  shift_vec.b8);    CMT("vl[0] <<= shift_vec;");
        i_and (r, dl[0].b8,    dl[0].b8,  bitmask_vec);     CMT("vl[0] &= bitmask_vec;");
    }
}
...
static RasmOp swizzle_a64op(SwsAArch64OpRegs *regs, int8_t n, uint8_t vh, bool dst)
{
    if (n == -1)
        return regs->vt[vh];
    return vh ? (dst ? regs->dh[n] : regs->sh[n]) : (dst ? regs->dl[n] : regs->sl[n]);
}
                             ...
                             SwsAArch64OpRegs *regs)
{
    ...
    LOOP_MASK_BWD      (p, i) { i_and(r, dl[i], dl[i], vmask[i]); CMTF("vl[%u] &= 0x%x;", i, (1u << p->par.pack.pattern[i]) - 1); }
    LOOP_MASK_BWD_VH(s, p, i) { i_and(r, dh[i], dh[i], vmask[i]); CMTF("vh[%u] &= 0x%x;", i, (1u << p->par.pack.pattern[i]) - 1); }
}
                           ...
                           SwsAArch64OpRegs *regs)
{
    ...
    /* Perform left shift. */
    LOOP      (offset_mask, i) { i_shl(r, dl[i], sl[i], IMM(offsets[i])); CMTF("vl[%u] <<= %u;", i, offsets[i]); sl[i] = dl[i]; }
    LOOP_VH(s, offset_mask, i) { i_shl(r, dh[i], sh[i], IMM(offsets[i])); CMTF("vh[%u] <<= %u;", i, offsets[i]); sh[i] = dh[i]; }
    ...
    LOOP_MASK      (p, i) {
        if (i != 0) {
            i_orr    (r, dl[0], sl[0], sl[i]); CMTF("vl[0] |= vl[%u];", i);
            if (s->use_vh) {
                i_orr(r, dh[0], sh[0], sh[i]); CMTF("vh[0] |= vh[%u];", i);
            }
        }
    }
}
...
static void linear_pass(SwsAArch64Context *s, const SwsAArch64OpImplParams *p,
                        RasmOp *vx, char cvh, RasmOp *vt, const RasmOp *vc,
                        SwsCompMask save_mask)
{
    RasmContext *r = s->rctx;
    /**
                             ...
                             SwsAArch64OpRegs *regs)
{
    ...
    linear_pass    (s, p, sl, 'l', vt, vc, save_mask);
    if (s->use_vh)
        linear_pass(s, p, sh, 'h', vt, vc, save_mask);
}
