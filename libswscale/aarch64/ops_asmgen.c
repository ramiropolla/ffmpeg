                        ...
                        SwsAArch64OpRegs *regs, bool vh_pass)
{
    ...
    LOOP_MASK(p, i) {
        ...
        for (int j = 0; j < 5; j++) {
            ...
            i_coeff++;
            if (first && is_offset) {
                i_dup (r, dx[i], vcoeff);               CMTF("v%c[%u]  = broadcast(vc[%u][%u]);", cvh, i, i, j);
            } else if (first && !is_offset) {
                if (p->par.lin.one & SWS_MASK(i, src_j)) {
                    i_mov16b(r, dx[i], vsrc);           CMTF("v%c[%u]  = vsrc%c[%u];", cvh, i, cvh, src_j);
                } else {
                    i_fmul  (r, dx[i], vsrc, vcoeff);   CMTF("v%c[%u]  = vsrc%c[%u] * vc[%u][%u];", cvh, i, cvh, src_j, i, j);
                }
                 ...
                 */
                i_fmla(r, dx[i], vsrc, vcoeff);         CMTF("v%c[%u] += vsrc%c[%u] * vc[%u][%u];", cvh, i, cvh, src_j, i, j);
            } else {
                /**
