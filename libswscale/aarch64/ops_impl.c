...
/**
 * This file is used both by the aarch64 backend to generate ops_entries.c
 * and by the standalone build-time tool that generates the static assembly
...
void sws_aarch64_op_impl_func_name(char *buf, size_t size, const SwsAArch64OpImplParams *params)
{
    ...
    assert(size && "string buffer exhausted");
}
