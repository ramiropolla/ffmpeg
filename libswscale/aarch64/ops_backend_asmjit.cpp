...
struct AsmJitContext {
    ...
    AsmJitContext(const SwsContext *swsctx, int block_size)
      : m_block_size(block_size)
    {
        ...
        /* HACK to set function name in asmjit */
        {
            ...
            snprintf(m_func_name, sizeof(m_func_name), "asmjit_%s_to_%s_neon",
                     av_get_pix_fmt_name((AVPixelFormat) swsctx->src_format),
                     av_get_pix_fmt_name((AVPixelFormat) swsctx->dst_format));
    ...
    void emit_loop()
    {
        ...
        for (int i = 0; i < (m_read_op->rw.packed ? 1 : m_read_op->rw.elems); i++) {
            if (!m_read_next_op->comps.unused[i]) {
        ...
        if (xy_unused) {
            cc.subs(m_x.r32(), m_x.r32(), m_block_size);
            cc.b(a64::CondCode::kGT, hloop);
        } else {
            cc.add(m_x.r32(), m_x.r32(), m_block_size);
        ...
        if (xy_unused) {
            cc.subs(m_y.r32(), m_y.r32(), 1);
            cc.b(a64::CondCode::kGT, vloop);
        } else {
            cc.add(m_y.r32(), m_y.r32(), 1);
...
static int emit_convert(AsmJitContext *ctx, int block_size, const SwsOp *next, SwsPixelType from, SwsPixelType to, bool expand)
{
    a64::Compiler &cc = *ctx->m_cc;
    a64::Vec *orig_vl = ctx->m_orig_vl;
    a64::Vec *orig_vh = ctx->m_orig_vh;
    ...
    if (expand) {
        snprintf(buf, sizeof(buf), "expand(%s -> %s, block_w %d)", ff_sws_pixel_type_name(from), ff_sws_pixel_type_name(to), block_size);
        cc.comment(buf);
        if (from_size == 1) {
            LOOP_OUT(i) {
...
if (use_vh) {
    ...
    case SWS_OP_CONVERT:         /* convert (cast) between formats */
        if (op.type != op.convert.to && emit_convert(ctx, block_size, next, op.type, op.convert.to, op.convert.expand) < 0)
            return AVERROR(ENOTSUP);
        break;
...
static av_cold int asmjit_compile(SwsContext *swsctx, SwsOpList *ops, SwsCompiledOp *out)
{
    ...
    AsmJitContext *ctx = new AsmJitContext(swsctx, block_size);
    a64::Compiler &cc = *ctx->m_cc;
    Error err;
    int ret;
