...
{
    if (r < 0) {
        uint32_t avail = ~(f->gpr_used | AARCH64_GPR_PR | AARCH64_GPR_SP);
        av_assert0(avail);
        r = ff_ctz(avail);
    } else {
        av_assert0(r >= 0 && r <= 30);
                   ...
                   RasmNode *prologue, RasmNode *epilogue)
{
    ...
    for (unsigned i = 0; i <= 30; i++) {
        if (f->gpr_clobbered & AARCH64_GPR_CALLEE_SAVED & (1u << i))
            regs[n++] = a64op_gpx(i);
    }
    if (n & 1)
        regs[n++] = rasm_op_none();
    for (unsigned i = 0; i <= 31; i++) {
        if (f->vec_clobbered & AARCH64_VEC_CALLEE_SAVED & (1u << i))
            regs[n++] = a64op_vecd(i);
    }
    if (n & 1)
        regs[n++] = rasm_op_none();
