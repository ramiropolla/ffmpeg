...
/**
 * When register exhaustion happens, the error code -1 will cast to
 * register number 255.
 */
#define AARCH64_REG_EXHAUSTED -1
...
static int regstate_exhausted(AArch64RegState *rs)
{
    rs->error = AARCH64_REG_EXHAUSTED;
    return rs->error;
}
