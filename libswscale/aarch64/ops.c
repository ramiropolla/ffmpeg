...
static void aarch64_impl_params(const SwsOpList *ops, int block_size, int n, SwsAArch64OpImplParams *out)
{
    ...
    switch (op->op) {
    case SWS_OP_READ:       out->op = AARCH64_SWS_OP_READ;       break;
    case SWS_OP_WRITE:      out->op = AARCH64_SWS_OP_WRITE;      break;
    ...
    case AARCH64_SWS_OP_WRITE:
        if (op->rw.frac == 1)
            out->rw = AARCH64_SWS_OP_RW_BIT;
        else if (op->rw.frac == 3)
            out->rw = AARCH64_SWS_OP_RW_NIBBLE;
        else if (op->rw.packed && op->rw.elems != 1)
            out->rw = AARCH64_SWS_OP_RW_PACKED;
        else
            out->rw = AARCH64_SWS_OP_RW_PLANAR;
        switch (op->rw.elems) {
