                               ...
                               SwsCompiledOp *out)
{
    ...
    /* create process */
    /* Op lists with no read (e.g. a pure CLEAR fill, used when the source
     * format lacks planes the destination needs) have no input pointers to
     * set up; params[0].mask in that case describes CLEAR's destination
     * components, not input planes, so it must not be used as such. */
    static const SwsAArch64OpImplParams no_input_params = { 0 };
    const SwsAArch64OpImplParams *pin = ff_sws_op_list_input(ops) ? &params[0] : &no_input_params;
