                             ...
                             const SwsImplResult *res, SwsAArch64OpRegs *regs, int i, SwsCompMask imask, SwsCompMask omask)
{
    ...
    case SWS_UOP_WRITE_PLANAR:
        LOOP_MASK      (p, i) { regs[0].sl[i] = a64frame_vec(&s->frame, -1); }
        LOOP_MASK_VH(s, p, i) { regs[0].sh[i] = a64frame_vec(&s->frame, -1); }
    ...
    case SWS_UOP_CLEAR:
        ...
        memcpy(regs[0].sl, regs[0].dl, sizeof(regs[0].sl));
        memcpy(regs[0].sh, regs[0].dh, sizeof(regs[0].sh));
        LOOP_MASK      (p, i) { a64frame_vec_free(&s->frame, regs[0].sl[i]); regs[0].sl[i] = rasm_op_none(); }
        LOOP_MASK_VH(s, p, i) { a64frame_vec_free(&s->frame, regs[0].sh[i]); regs[0].sh[i] = rasm_op_none(); }
        break;
    ...
    case SWS_UOP_TO_F32: {
        ...
        if (p->block_size == 8) {
            if        (src_el_size == 1 && dst_el_size == 2) {
                ...
                /* 8 -> 32 */
                LOOP_MASK(p, i) { a64frame_vec_free(&s->frame, regs[0].sh[i]); regs[0].sh[i] = rasm_op_none(); }
                ...
                /* 32 -> 8 */
                LOOP_MASK(p, i) { regs[0].sh[i] = a64frame_vec(&s->frame, -1); }
