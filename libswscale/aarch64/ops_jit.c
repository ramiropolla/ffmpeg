                              ...
                              SwsCompMask imask, SwsCompMask omask, AVBPrint *errstr)
{
    ...
    if (ctx->s.regstate.error) {
        av_log(sws, AV_LOG_WARNING, "JIT register allocation exhausted\n");
    ...
    if (ctx->s.data_count)
        av_bprintf(&bp, "%s\n", asm_const_macros);
    int ret = rasm_print(ctx->s.rctx, &bp);
    if (ret < 0)
        goto error;
    ...
    ret = ff_sws_jit_assemble_llvm(sws, bp.str, &ctx->code, &ctx->code_size, errstr);
