...
%macro convert_8_16 4 ; reg, reg2, xreg, xreg2
IF V2,  vextracti128 %4, %1, 1
        pmovzxbw %1, %3
IF V2,  pmovzxbw %2, %4
%endmacro

%macro convert_16_8 4 ; reg, reg2, xreg, xreg2
%if V2
        packuswb %1, %2
        vpermq %1, %1, q3120
%else
...
%macro convert_8_32 4 ; reg, reg2, xreg, xreg2
        psrldq %4, %3, 8
        pmovzxbd %1, %3
        pmovzxbd %2, %4
%endmacro

%macro convert_32_8 4 ; reg, reg2, xreg, xreg2
        packusdw %1, %2
        vextracti128 %4, %1, 1
        packuswb %3, %4
        vpshufd %3, %3, q3120
...
%macro convert_16_32 4 ; reg, reg2, xreg, xreg2
        vextracti128 %4, %1, 1
        pmovzxwd %1, %3
        pmovzxwd %2, %4
%endmacro

%macro convert_32_16 4 ; reg, reg2, xreg, xreg2
        packusdw %1, %2
        vpermq %1, %1, q3120
%endmacro
...
%macro CONVERT 1
%xdefine TO %1
%ifidn   TO, SWS_PIXEL_U8
    %assign BITS_TO 8
%elifidn TO, SWS_PIXEL_U16
    %assign BITS_TO 16
%else
    %assign BITS_TO 32
%endif
...
IF1 X,  convert_ %+ BITS %+ _ %+ BITS_TO mx, mx2, xmx, xmx2
IF1 Y,  convert_ %+ BITS %+ _ %+ BITS_TO my, my2, xmy, xmy2
IF1 Z,  convert_ %+ BITS %+ _ %+ BITS_TO mz, mz2, xmz, xmz2
IF1 W,  convert_ %+ BITS %+ _ %+ BITS_TO mw, mw2, xmw, xmw2
    %if cpuflag(avx2) && (BITS > BITS_TO && !V2 || BITS >= BITS_TO * 4)
        ; clear upper bits after reducing the register size
        vzeroupper ; TMP
    %endif
%endif
%ifidn TO, SWS_PIXEL_F32
...
%endif
        ...
        LOAD_CONT tmp0q
IF1 X,  convert_8_ %+ BITS_TO mx, mx2, xmx, xmx2
IF1 Y,  convert_8_ %+ BITS_TO my, my2, xmy, xmy2
IF1 Z,  convert_8_ %+ BITS_TO mz, mz2, xmz, xmz2
IF1 W,  convert_8_ %+ BITS_TO mw, mw2, xmw, xmw2
IF X,   pshufb mx, m8
IF Y,   pshufb my, m8
IF Z,   pshufb mz, m8
IF W,   pshufb mw, m8
...
%macro decl_ops 1 ; type
    DECL_READ_PACKED_%1     (READ_PACKED)
    DECL_READ_NIBBLE_%1     (READ_NIBBLE)
    DECL_READ_BIT_%1        (READ_BIT)
    DECL_WRITE_PACKED_%1    (WRITE_PACKED)
    DECL_WRITE_NIBBLE_%1    (WRITE_NIBBLE)
    DECL_WRITE_BIT_%1       (WRITE_BIT)
    DECL_PERMUTE_%1         (PERMUTE_CYCLES)
    DECL_COPY_%1            (COPY)
    DECL_SWAP_BYTES_%1      (SWAP_BYTES)
    DECL_EXPAND_BIT_%1      (EXPAND_BIT)
    DECL_SCALE_%1           (SCALE)
    DECL_ADD_%1             (ADD)
    DECL_MIN_%1             (MIN)
    DECL_MAX_%1             (MAX)
    DECL_UNPACK_%1          (UNPACK)
    DECL_PACK_%1            (PACK)
    DECL_LSHIFT_%1          (LSHIFT)
    DECL_RSHIFT_%1          (RSHIFT)
    DECL_LINEAR_FMA_%1      (LINEAR_FMA)
    DECL_DITHER_%1          (DITHER)
%endmacro

%macro decl_type_invariant 0
    DECL_READ_PLANAR_U8     (READ_PLANAR)
    DECL_WRITE_PLANAR_U8    (WRITE_PLANAR)
    DECL_CLEAR_U8           (CLEAR)
%endmacro

%macro decl_convert_u16 0
    DECL_CONVERT_U8_U16     (CONVERT)
    DECL_CONVERT_U16_U8     (CONVERT)
    DECL_EXPAND_PAIR_U8     (EXPAND_BYTE)
%endmacro

%macro decl_convert_u32 0
    DECL_CONVERT_U8_U32     (CONVERT)
    DECL_CONVERT_U32_U8     (CONVERT)
    DECL_CONVERT_U16_U32    (CONVERT)
    DECL_CONVERT_U32_U16    (CONVERT)
    DECL_EXPAND_QUAD_U8     (EXPAND_BYTE)
%endmacro

%macro decl_convert_f32 0
    DECL_CONVERT_U8_F32     (CONVERT)
    DECL_CONVERT_F32_U8     (CONVERT)
    DECL_CONVERT_U16_F32    (CONVERT)
    DECL_CONVERT_F32_U16    (CONVERT)
    DECL_CONVERT_U32_F32    (CONVERT)
    DECL_CONVERT_F32_U32    (CONVERT)
%endmacro
...
decl_v2 0, decl_type_invariant
decl_v2 1, decl_type_invariant
decl_v2 0, decl_convert_u16
decl_v2 1, decl_convert_u16

decl_v2 1, decl_ops U32
decl_v2 1, decl_convert_u32
decl_v2 1, decl_convert_f32
