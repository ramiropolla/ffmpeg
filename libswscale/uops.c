...
void ff_sws_uop_name(const SwsUOp *op, char buf[SWS_UOP_NAME_MAX])
{
    ...
    case SWS_UOP_CONVERT:
        av_bprintf(&bp, "_%s", ff_sws_pixel_type_name(par->convert.to));
        break;
    ...
    case SWS_UOP_LINEAR:
        ...
        break;
    case SWS_UOP_DITHER:
        av_bprintf(&bp, "_%d", par->dither.size_log2);
        for (int i = 0; i < 4; i++) {
            if (SWS_COMP_TEST(op->mask, i))
...
static int generate_entry_struct(void *opaque, void *key)
{
    ...
    av_bprintf(bp, ", .uop = %-24s, .type = %-13s, .mask = 0x%x",
    ...
    case SWS_UOP_RSHIFT:
        ...
        break;
    case SWS_UOP_CONVERT:
        av_bprintf(bp, ", .par.convert.to = %s", pixel_types[par->convert.to].full);
        break;
    case SWS_UOP_PERMUTE:
    case SWS_UOP_COPY:
    ...
    case SWS_UOP_LINEAR:
        ...
        break;
    case SWS_UOP_DITHER:
        av_bprintf(bp, ", .par.dither = { .size_log2 = %u, .y_offset = {%u, %u, %u, %u} }",
...
static int generate_entry_args(void *opaque, void *key)
{
    ...
    case SWS_UOP_RSHIFT:
        ...
        break;
    case SWS_UOP_CONVERT:
        av_bprintf(bp, ", %s", pixel_types[par->convert.to].full);
        break;
    case SWS_UOP_PERMUTE:
    case SWS_UOP_COPY:
    ...
    case SWS_UOP_LINEAR:
        ...
        break;
    case SWS_UOP_DITHER:
        av_bprintf(bp, ", %u, %u, %u, %u, %u", par->dither.size_log2,
                   par->dither.y_offset[0], par->dither.y_offset[1],
                   par->dither.y_offset[2], par->dither.y_offset[3]);
        break;
    }
...
static SwsPixelType pixel_to_int(const SwsPixelType type)
{
    switch (ff_sws_pixel_type_size(type)) {
...
static int translate_dither_op(SwsUOpList *ops, const SwsOp *op)
{
    ...
    if (size == 1) {
        /* Backends may hard-code this as a fast path */
...
static const SwsUOpType uop_map[SWS_OP_TYPE_NB] = {
    [SWS_OP_SWAP_BYTES] = SWS_UOP_SWAP_BYTES,
    [SWS_OP_UNPACK]     = SWS_UOP_UNPACK,
    [SWS_OP_PACK]       = SWS_UOP_PACK,
    [SWS_OP_LSHIFT]     = SWS_UOP_LSHIFT,
    [SWS_OP_RSHIFT]     = SWS_UOP_RSHIFT,
    [SWS_OP_CLEAR]      = SWS_UOP_CLEAR,
    [SWS_OP_MIN]        = SWS_UOP_MIN,
    [SWS_OP_MAX]        = SWS_UOP_MAX,
    [SWS_OP_DITHER]     = SWS_UOP_DITHER,
};
...
static int translate_op(SwsUOpList *uops, const SwsOp *op)
{
    ...
    case SWS_OP_CONVERT:
        if (op->convert.expand) {
            ...
            uop.uop = SWS_UOP_CONVERT;
            uop.par.convert.to = op->convert.to;
        }
        break;
    case SWS_OP_UNPACK:
    case SWS_OP_PACK:
...
"\n");
    ...
    for (key.type = SWS_PIXEL_NONE + 1; key.type < SWS_PIXEL_TYPE_NB; key.type++) {
        for (key.uop = SWS_UOP_INVALID + 1; key.uop < SWS_UOP_TYPE_NB; key.uop++) {
            ...
            av_bprintf(bp, "#define FOR_%s%s(MACRO, ...)", uop, suffix);
            av_tree_enumerate(root, &key, enum_type, generate_entry_args);
            av_bprintf(bp, "\n");
            av_bprintf(bp, "#define FOR_STRUCT_%s%s(MACRO, ...)", uop, suffix);
             ...
             key.par.convert.to++)
        {
            ...
            av_bprintf(bp, "#define FOR_%s%s%s(MACRO, ...)", uop, from, to);
            av_tree_enumerate(root, &key, enum_type, generate_entry_args);
            av_bprintf(bp, "\n");
            av_bprintf(bp, "#define FOR_STRUCT_%s%s%s(MACRO, ...)", uop, from, to);
            av_tree_enumerate(root, &key, enum_type, generate_entry_struct);
            av_bprintf(bp, "\n");
        }
