...
SWS_FOR(READ_PLANAR,    PX, DECL_IMPL_READ,     read_planar)
SWS_FOR(READ_PACKED,    PX, DECL_IMPL_READ,     read_packed)
SWS_FOR(READ_NIBBLE,    PX, DECL_IMPL_READ,     read_nibble)
SWS_FOR(READ_BIT,       PX, DECL_IMPL_READ,     read_bit)
SWS_FOR(WRITE_PLANAR,   PX, DECL_IMPL_WRITE,    write_planar)
SWS_FOR(WRITE_PACKED,   PX, DECL_IMPL_WRITE,    write_packed)
SWS_FOR(WRITE_NIBBLE,   PX, DECL_IMPL_WRITE,    write_nibble)
SWS_FOR(WRITE_BIT,      PX, DECL_IMPL_WRITE,    write_bit)

SWS_FOR_STRUCT(READ_PLANAR,     PX, DECL_ENTRY)
SWS_FOR_STRUCT(READ_PACKED,     PX, DECL_ENTRY)
SWS_FOR_STRUCT(READ_NIBBLE,     PX, DECL_ENTRY)
SWS_FOR_STRUCT(READ_BIT,        PX, DECL_ENTRY)
SWS_FOR_STRUCT(WRITE_PLANAR,    PX, DECL_ENTRY)
SWS_FOR_STRUCT(WRITE_PACKED,    PX, DECL_ENTRY)
SWS_FOR_STRUCT(WRITE_NIBBLE,    PX, DECL_ENTRY)
SWS_FOR_STRUCT(WRITE_BIT,       PX, DECL_ENTRY)
...
SWS_FOR(READ_PLANAR_FV, PX, DECL_IMPL_READ, read_planar_fv)
SWS_FOR(READ_PLANAR_FH, PX, DECL_IMPL_READ, read_planar_fh)
SWS_FOR_STRUCT(READ_PLANAR_FV, PX, DECL_ENTRY, .setup = fn(setup_filter_v) )
SWS_FOR_STRUCT(READ_PLANAR_FH, PX, DECL_ENTRY, .setup = fn(setup_filter_h) )
...
#define DECL_PERMUTE(DUMMY, NAME, UOP, TYPE, MASK, IDX0, IDX1, IDX2, IDX3)      \
...
#define DECL_COPY(DUMMY, NAME, UOP, TYPE, MASK, IDX0, IDX1, IDX2, IDX3)         \
    static void NAME##_c(SwsOpIter *restrict iter,                              \
                         const SwsOpImpl *restrict impl,                        \
                         void *restrict in0, void *restrict in1,                \
                         void *restrict in2, void *restrict in3)                \
...
SWS_FOR(PERMUTE, PX, DECL_PERMUTE)
SWS_FOR(COPY,    PX, DECL_COPY)
SWS_FOR_STRUCT(PERMUTE, PX, DECL_ENTRY)
SWS_FOR_STRUCT(COPY,    PX, DECL_ENTRY)

/*********************
 * Format conversion *
 *********************/

#define DECL_CONVERT(TO, TYPE)                                                  \
DECL_FUNC(convert_##TO, const SwsCompMask mask, const SwsPixelType to)          \
{                                                                               \
    block_t xx, yy, zz, ww;                                                     \
                                                                                \
    SWS_LOOP                                                                    \
    ...
    CONTINUE(&xx, &yy, &zz, &ww);                                               \
                                                                                ...
                                                                                \
SWS_FOR_CONVERT(PX, TYPE, DECL_IMPL, convert_##TO)                              \
SWS_FOR_STRUCT_CONVERT(PX, TYPE, DECL_ENTRY)

DECL_CONVERT(u8,  U8)
DECL_CONVERT(u16, U16)
DECL_CONVERT(u32, U32)
DECL_CONVERT(f32, F32)
...
SWS_FOR(LSHIFT, PX, DECL_IMPL, lshift)
SWS_FOR(RSHIFT, PX, DECL_IMPL, rshift)

SWS_FOR_STRUCT(LSHIFT, PX, DECL_ENTRY)
SWS_FOR_STRUCT(RSHIFT, PX, DECL_ENTRY)
...
SWS_FOR(SWAP_BYTES, PX, DECL_IMPL, swap_bytes)
SWS_FOR_STRUCT(SWAP_BYTES, PX, DECL_ENTRY)
...
SWS_FOR(EXPAND_BIT,  PX, DECL_IMPL, expand_bit)
SWS_FOR(EXPAND_PAIR, PX, DECL_IMPL, expand_pair)
SWS_FOR(EXPAND_QUAD, PX, DECL_IMPL, expand_quad)
SWS_FOR_STRUCT(EXPAND_BIT,  PX, DECL_ENTRY)
SWS_FOR_STRUCT(EXPAND_PAIR, PX, DECL_ENTRY)
SWS_FOR_STRUCT(EXPAND_QUAD, PX, DECL_ENTRY)
...
SWS_FOR(UNPACK, PX, DECL_IMPL, unpack)
SWS_FOR(PACK,   PX, DECL_IMPL, pack)
SWS_FOR_STRUCT(UNPACK,  PX, DECL_ENTRY)
SWS_FOR_STRUCT(PACK,    PX, DECL_ENTRY)
...
SWS_FOR(CLEAR, PX, DECL_IMPL, clear)
SWS_FOR_STRUCT(CLEAR, PX, DECL_ENTRY, .setup = ff_sws_setup_vec4)
...
SWS_FOR(SCALE, PX, DECL_IMPL, scale)
SWS_FOR(ADD,   PX, DECL_IMPL, add)
SWS_FOR(MIN,   PX, DECL_IMPL, min)
SWS_FOR(MAX,   PX, DECL_IMPL, max)
SWS_FOR_STRUCT(SCALE, PX, DECL_ENTRY, .setup = ff_sws_setup_scalar )
SWS_FOR_STRUCT(ADD,   PX, DECL_ENTRY, .setup = ff_sws_setup_vec4 )
SWS_FOR_STRUCT(MIN,   PX, DECL_ENTRY, .setup = ff_sws_setup_vec4 )
SWS_FOR_STRUCT(MAX,   PX, DECL_ENTRY, .setup = ff_sws_setup_vec4 )
...
DECL_FUNC(dither, const SwsCompMask mask, const uint8_t size_log2,
...
SWS_FOR(DITHER, PX, DECL_IMPL, dither)
SWS_FOR_STRUCT(DITHER, PX, DECL_ENTRY, .setup = fn(setup_dither) )
...
SWS_FOR(LINEAR, PX, DECL_IMPL, linear)
SWS_FOR_STRUCT(LINEAR, PX, DECL_ENTRY, .setup = fn(setup_linear) )
