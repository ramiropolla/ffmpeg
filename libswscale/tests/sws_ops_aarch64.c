...
static void impl_func_name(AVBPrint *bp, const SwsAArch64OpImplParams *params)
{
    ...
    case SWS_UOP_LINEAR:
    case SWS_UOP_LINEAR_FMA:
        av_bprintf(bp, "_%010" PRIx64 "_%d", linear_to_mask(&par->lin), params->uop == SWS_UOP_LINEAR_FMA);
        break;
    case SWS_UOP_DITHER:
...
static void serialize_op(AVBPrint *bp, const SwsAArch64OpImplParams *params)
{
    const SwsUOpParams *par = &params->par;
    char buf1[32];
    snprintf(buf1, sizeof(buf1), "%s,", op_types[params->uop]);
    char buf2[32];
    snprintf(buf2, sizeof(buf2), "%s,", pixel_types[params->type]);
    av_bprintf(bp, "{ .uop = %-21s .block_size = %u, .type = %-14s .mask = 0x%x", buf1, params->block_size, buf2, params->mask);
    switch (params->uop) {
    case SWS_UOP_MOVE:
...
static void serialize_entry(AVBPrint *bp, const SwsAArch64OpImplParams *params)
{
    av_bprintf(bp, "ENTRY(");
...
static int print_entry(void *opaque, void *elem)
{
...
bad_option:
    ...
    if (src_fmt == AV_PIX_FMT_NONE && dst_fmt == AV_PIX_FMT_NONE) {
        ...
        printf(" */\n");
        printf("\n");
        av_tree_enumerate(root, stdout, NULL, print_entry);
    } else {
        SwsOpList *ops = NULL;
